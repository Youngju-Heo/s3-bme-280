...
| 센서 연결 | SPI2, 모드 0, 10MHz, SCK=GPIO12 / MOSI=GPIO11 / CS=GPIO10 / MISO=GPIO13 | 사용자 요청(SPI 테스트). SPI2의 IO_MUX 전용 핀을 사용해 GPIO 매트릭스를 거치지 않음(최대 80MHz, BME280 상한 10MHz). 초기 구현은 GND/3V3 쪽 GPIO3~6·1MHz였으나 고속 통신 실험을 위해 변경 |
...
| main 소스 파일 | `app-main.c`, `sampler.c`, `protocol.c`, `json-mini.c`, `clock.c`, `settings.c`, `spi-bus.c`, `status-led.c`, `status-led-color.c` |
