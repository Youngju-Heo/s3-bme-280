...
from .records import CSV_HEADER, DEFAULT_SEA_LEVEL_HPA, LogRecord, SyncInfo, altitude_m, csv_row, format_time, resolve_time
...
def cmd_now(client: DeviceClient, sync: SyncInfo, args) -> int:
    ...
    print(f"고도: {altitude_m(r['pressure_pa'], args.sea_level):.1f} m")
    return 0
...
def cmd_log(client: DeviceClient, sync: SyncInfo, args) -> int:
    ...
    if args.csv:
        with open(args.csv, "w", newline="", encoding="utf-8") as f:
            ...
            for r in records:
                writer.writerow(csv_row(r, sync, args.sea_level))
        print(f"{len(records)}건을 {args.csv}에 저장했습니다")
        return 0

    print(f"{'시각':<22}{'온도(°C)':>10}{'습도(%)':>10}{'기압(hPa)':>12}{'고도(m)':>10}")
    for r in records:
        print(f"{format_time(r, sync):<22}{r.temp_c:>10.2f}{r.hum_pct:>10.2f}{r.pressure_hpa:>12.2f}"
              f"{altitude_m(r.pressure_pa, args.sea_level):>10.1f}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--sea-level", type=float, default=DEFAULT_SEA_LEVEL_HPA, dest="sea_level",
                   help=f"고도 계산용 해면 기압 hPa (기본 {DEFAULT_SEA_LEVEL_HPA})")
